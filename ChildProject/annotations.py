...
class AnnotationManager:
    ...
    def summarise_conversations(self,
        ...
        if threads == 1:
            imported = input_processed.apply(
                partial(self._summarise_conversations,
                        overwrite_existing=overwrite_existing
                        ), axis=1
            ).to_dict(orient="records")
        else:
        ...
        if 'error' in imported.columns:
            ...
            if errors.shape[0] > 0:
                output = os.path.join(self.project.path, "extra",
                                      "errors_conv_summary_{}.csv".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")))
                errors.to_csv(output, index=False)
                logger_annotations.info("Errors summary exported to %s", output)
        else:
            errors = None
