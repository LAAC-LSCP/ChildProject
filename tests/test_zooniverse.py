...
def test_extraction():
    ...
    assert all(chunks['wav'].apply(lambda f: os.path.exists(os.path.join('output/zooniverse/chunks', f))).tolist())
    assert all(chunks['mp3'].apply(lambda f: os.path.exists(os.path.join('output/zooniverse/chunks', f))).tolist())
